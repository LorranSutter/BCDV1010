...
contract("Faucet", (accounts) => {
    ...
    const withdrawalAmount = 2*10**15;
    const refillAmount = 10**16;
    let faucetInstance;
    ...
    before(async () => {
        ...
        assert.equal(refillAmount, balance, "The balance of the contract should be as expected");
    });
    ...
    it("should be able to withdraw using withdraw()", async () => {
        ...
        const tx = await faucetInstance.withdraw(web3.utils.toBN(withdrawalAmount), { from: beneficiary });

        // Obtain the gas price
        const trx = await web3.eth.getTransaction(tx.receipt.transactionHash);
        const transactionFee = web3.utils.toBN(trx.gasPrice).mul(web3.utils.toBN(tx.receipt.gasUsed));
        ...
        truffleAssert.eventEmitted(tx, "Withdrawal", (obj) => {
            return obj.to === beneficiary && new BigNumber(obj.amount).isEqualTo(new BigNumber(withdrawalAmount));
        });
    });

    it("should revert on withdraw if amount exceeds the contract balance", async () => {
        const withdrawalAmount = web3.utils.toBN(refillAmount).add(web3.utils.toBN(2));
